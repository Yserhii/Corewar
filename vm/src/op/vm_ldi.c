...
void	vm_ldi(t_vm *vm, t_kar *kar)
{
	...
	while (kod[0] && (kod[1] == REG_CODE || kod[1] == DIR_CODE) && kod[2] == REG_CODE && i--) // && !kod[3]
	{
		...
		if ((kod[0] == REG_CODE && (int)args[0] == -1) ||
			...
			break;
		ind = kar->pos + (args[0] + args[1]) % IDX_MOD;
		kar->reg[(int)args[2]] = take_arg(vm, (ind % MEM_SIZE), 4);
		/// тут нужно керри или нет я хз в описание ни че не сказано про кери и в
		// бражнике тоже. Хотя в функции lldi в сабжакте сказано про керрии
		// а в бражнике нет
